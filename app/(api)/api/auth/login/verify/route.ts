...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action !== 'authenticate') {
      return NextResponse.json({ message: AuthMessages.INVALID_OTP_ACTION }, { status: 400 })
    }
    ...
    if (method === 'TOTP_APP') {
      ...
      if (!isTOTPValid) {
        return NextResponse.json({ message: AuthMessages.INVALID_TOKEN }, { status: 400 })
      }
    } else {
      await OTPService.verifyOTP({ user, userSession, method, otpToken, action })
    }

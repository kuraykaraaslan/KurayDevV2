...
export async function GET(req: Request) {
  ...
  const projectResults = ProjectService.getAllProjects({ page: 0, pageSize: 2, search: q }).then(
    (res) => {
      return res.projects
    }
  )

  const [blogData, projectData] = (await Promise.all([blogResults, projectResults])) as [
    PostWithData[],
    Project[],
  ]
  ...
  const projectResultsFormatted: SearchResultItemType[] = projectData
    .map((project) => ({
      title: project.title,

...
export default class WebAuthnService {
    ...
    static async verifyRegistration({
        ...
        // Do NOT re-encode it — Buffer.from(string) would produce wrong bytes.
    ...
    static async verifyAuthentication({
        ...
        console.log('Verifying authentication response for credentialId:', response)
        console.log('Authentication response email:', email)

        // ── 1. Find the user that owns this credential ────────────────────────
        // jsonb_array_elements lets us filter by credentialId inside the array.
        const rows = await prisma.$queryRawUnsafe<{ userId: string }[]>(`
            SELECT "userId"
            FROM "User"
            WHERE EXISTS (
                SELECT 1
                FROM jsonb_array_elements("userSecurity"->'passkeys') AS pk
        ...
        console.log('Stored passkey found:', storedPasskey ?? 'NOT FOUND', '| response.id:', response.id)
        if (!storedPasskey) throw new Error(AuthMessages.PASSKEY_NOT_REGISTERED)

        // ── 2. Retrieve the challenge ─────────────────────────────────────────
        // Email-scoped flow stores under userId key.
        // Resident-key (conditional UI) flow stores under the '_resident_' key.

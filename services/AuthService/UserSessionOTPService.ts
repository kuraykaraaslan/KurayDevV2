...
export default class UserSessionOTPService {
  ...
  static async rateLimitGuard(sessionId: string, method: OTPMethod) {
    const key = OTP_RATE_KEY(sessionId, method)
    const exists = await redis.get(key)
    if (exists) throw new Error(AuthMessages.OTP_ALREADY_SENT)
    await redis.set(key, '1', 'EX', OTP_RATE_LIMIT_SECONDS)
  }
  ...
  static async storeOTP(sessionId: string, method: OTPMethod, token: string, action: string) {
    ...
    const key = OTP_CODE_KEY(sessionId, method, action)
    await redis.set(key, hashed, 'EX', OTP_EXPIRY_SECONDS)
  }
  ...
  static async sendOTP({
    ...
    const otpKey = OTP_CODE_KEY(sessionId, method, 'login')
    const rateKey = OTP_RATE_KEY(sessionId, method)
    ...
    if (rateCount) {
      ...
      if (count >= OTP_RATE_LIMIT_MAX) {
        ...
        await redis.set(rateKey, (count + 1).toString(), 'EX', OTP_RATE_LIMIT_SECONDS)
      }
    } else {
    ...
    switch (method) {
      ...
      case OTPMethod.SMS:
        ...
        await SMSService.sendShortMessage({
          to: user.phone,
          body: `Your OTP code is ${token}. Valid for ${OTP_EXPIRY_SECONDS / 60} minutes.`,
        })
        break
  ...
  static async validateOTP({
    ...
    if (method === OTPMethod.TOTP_APP) {
      ...
      const key = OTP_CODE_KEY(userSession.userSessionId, method, action)
      ...
      await redis.del(key)
      await redis.del(OTP_RATE_KEY(userSession.userSessionId, `${method}:${action}`))
    }

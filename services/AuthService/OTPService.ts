...
export default class OTPService {
  ...
  static async requestOTP({
    ...
    const rateKey = this.getRedisKey({
  ...
  static async verifyOTP({
    ...
    await redis.del(
      this.getRedisKey({ userSessionId: userSession.userSessionId, method, action: 'rate' })
    )
  }

  static listOTPStatus(user: User): { active: OTPMethod[]; inactive: OTPMethod[] } {
    const all: OTPMethod[] = [
      OTPMethodEnum.Enum.EMAIL,
      OTPMethodEnum.Enum.SMS,
      OTPMethodEnum.Enum.TOTP_APP,
    ]
    return {

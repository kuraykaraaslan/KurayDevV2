...
const LanguageBar = ({
  ...
  const sourceLangOptions = allCurrentLangs.map((l) => ({ value: l, label: LANG_NAMES[l] ?? l }))
          ...
          <DynamicSelect
            ...
            renderSelected={(opt) => (
              <span className="flex items-center gap-1.5">
                <img src={getLangFlagUrl(opt.value as AppLanguage)} alt="" className="w-5 h-5 rounded-full shrink-0 select-none" />
                <span className="font-medium">{opt.value.toUpperCase()}</span>
                <span className="text-[10px] tracking-wider text-base-content/40">SRC</span>
              </span>
            )}
          />
                ...
                <div className="max-h-56 overflow-y-auto py-1">
                  {filteredAvailable.length === 0 ? (
                    ...
                    filteredAvailable.map((lang) => (
                      ...
                      >
                        <img src={getLangFlagUrl(lang as AppLanguage)} alt="" className="w-5 h-5 rounded-full shrink-0 select-none" />

...
export default function Tabs({
    ...
    const getContainerClasses = () => {
        switch (variant) {
            case 'boxed':
                return 'flex gap-2 p-1 bg-base-200 rounded-xl';
            case 'lifted':
                return 'flex gap-1 border-b border-base-300';
            case 'underline':
            default:
                return 'flex gap-2 border-b border-base-300';
        }
    };
            ...
            <div className={`${getContainerClasses()} ${tabsClassName}`}>
                {tabs.map((tab) => (
                    <button
                        ...
                        className={`flex items-center gap-2 ${sizeClasses[size]} font-medium transition-all duration-200 relative ${getVariantClasses(activeTab === tab.id)} ${tab.disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                        {tab.icon && (
